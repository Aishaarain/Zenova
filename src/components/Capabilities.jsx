...
function CapabilityVisual({ index }) {
  const visuals = [
    <div className="p-4 sm:p-6 space-y-4 w-full" key="web">
      <div className="flex gap-2">
        <div className="h-2.5 w-16 rounded" style={{ background: "var(--accent)" }} />
        <div className="h-2.5 w-10 rounded" style={{ background: "var(--border-strong)" }} />
      </div>
    ...
    <div className="p-4 sm:p-6 w-full flex flex-col items-center justify-center h-full gap-0" key="ai">
      ...
      <svg width="100%" height="36" viewBox="0 0 300 36" style={{ overflow: "visible" }} className="max-w-[260px] sm:max-w-none">
        <line x1="60" y1="0" x2="150" y2="30" stroke="var(--border-strong)" strokeWidth="1.5" />
        <line x1="150" y1="0" x2="150" y2="30" stroke="var(--accent)" strokeWidth="1.5" />
        <line x1="240" y1="0" x2="150" y2="30" stroke="var(--border-strong)" strokeWidth="1.5" />
      </svg>
      ...
      <div
        className="px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl text-xs font-semibold mb-3"
        style={{ background: "var(--accent)", color: "#0B0D12", fontFamily: "var(--font-display)" }}
      >
        Your Product
      </div>

      <svg width="100%" height="24" viewBox="0 0 300 24" style={{ overflow: "visible" }} className="max-w-[260px] sm:max-w-none">
        <line x1="150" y1="0" x2="150" y2="24" stroke="var(--border-strong)" strokeWidth="1.5" strokeDasharray="3 3" />
      </svg>
    ...
    <div className="p-4 sm:p-6 w-full" key="tools">
      <div className="grid grid-cols-4 gap-2 text-[10px] mb-2" style={{ color: "var(--text-faint)", fontFamily: "var(--font-mono)" }}>
        <span>Client</span>
        <span>Status</span>
        <span>Risk</span>
      ...
      {[
        ...
        <div key={i} className="grid grid-cols-4 gap-2 py-2 text-[10px] sm:text-xs" style={{ borderTop: "1px solid var(--border)", color: "var(--text-muted)" }}>
          <span style={{ color: "var(--text)" }}>{row[0]}</span>
          <span>{row[1]}</span>
          <span style={{ color: "var(--accent)" }}>{row[2]}</span>
          <span>2d ago</span>
    ...
    <div className="p-4 sm:p-6 space-y-4 w-full" key="seo">
      <div className="flex items-center justify-between">
        <span className="text-[10px] px-2 py-1 rounded" style={{ background: "var(--bg-alt)", color: "var(--text-faint)", fontFamily: "var(--font-mono)" }}>
          Lighthouse Report
        </span>
      ...
      <div className="grid grid-cols-4 gap-2 sm:gap-3">
        {[
          ...
          <div key={i} className="flex flex-col items-center gap-2">
            <div
              className="w-9 h-9 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-xs font-semibold"
              style={{
                background: `conic-gradient(var(--accent) ${score}%, var(--border) 0)`,
                color: "var(--text)",
              }}
...
export default function Capabilities() {
  ...
  return (
    <section id="How" className="px-6 md:px-10 pt-16 md:pt-20">
      <div className="max-w-7xl mx-auto mb-4">
        <Reveal>
          <Eyebrow>Capabilities</Eyebrow>
          <h2 className="text-3xl sm:text-4xl md:text-6xl font-semibold max-w-2xl" style={{ color: "var(--text)", fontFamily: "var(--font-display)" }}>
            How we work with you
          </h2>
        </Reveal>
      </div>
      ...
      {/* MOBILE / TABLET — text only, no visual panel */}
      <div className="md:hidden max-w-7xl mx-auto mt-8 space-y-6">
        {ITEMS.map((it) => (
          <Reveal key={it.n} className="border-l-2 pl-5 py-2" style={{ borderColor: "var(--accent)" }}>
            <div className="text-sm mb-1" style={{ color: "var(--accent)", fontFamily: "var(--font-mono)" }}>
              {it.n}
            </div>
            <h3 className="text-xl font-semibold mb-2" style={{ color: "var(--text)", fontFamily: "var(--font-display)" }}>
              {it.title}
            </h3>
      ...
      {/* DESKTOP — pinned scrollytelling effect */}
      <div
        ref={outerRef}
        style={{ height: `${ITEMS.length * 70}vh` }}
        className="hidden md:block relative max-w-7xl mx-auto mt-10"
      >
        <div className="sticky top-0 h-screen flex items-center">
          <div className="w-full grid grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              {ITEMS.map((it, i) => (
                <div
                  key={it.n}
                ...
                >
                  <div className="text-sm mb-2" style={{ color: "var(--accent)", fontFamily: "var(--font-mono)" }}>
                    {it.n}
                  </div>
                  <h3 className="text-2xl lg:text-3xl font-semibold mb-2" style={{ color: "var(--text)", fontFamily: "var(--font-display)" }}>
                    {it.title}
                  </h3>
                  {active === i && (
                    <p className="text-sm md:text-base leading-relaxed max-w-md" style={{ color: "var(--text-muted)" }}>
            ...
            <div className="relative h-[320px] lg:h-[380px] rounded-2xl overflow-hidden" style={{ border: "1px solid var(--border-strong)", background: "#080A0E" }}>
              {ITEMS.map((_, i) => (
                <div
                  key={i}
                  className="absolute inset-0 flex items-center justify-center"
